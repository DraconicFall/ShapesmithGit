    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (typeOfBullet == typeOfAtk.bullet)
        {
            ...
            if (notEnemyBullet == false)
            {
                ...
                {
                    if (collision.GetComponent<PlayerScript>() != null /*&& GameManagerScript.playerTookDmg == false && GameManagerScript.bossAnimationSignalReceivedINTRO == false*/)
                    {
        ...
        if (typeOfBullet == typeOfAtk.field)
        {
            if (notEnemyBullet == false)
            {
                if (collision.gameObject.tag == "Player")
                {
                    ...
                    if (collision.GetComponent<PlayerScript>() != null /*&& GameManagerScript.playerDashing == false && GameManagerScript.playerTookDmg == false && canDoDmg == true && GameManagerScript.bossAnimationSignalReceivedINTRO == false*/)
                    {
                        Debug.Log("hit");
